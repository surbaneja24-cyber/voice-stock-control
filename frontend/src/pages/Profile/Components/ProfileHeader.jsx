...
export default function ProfileHeader({ profile, isEditing, onToggleEdit, onAvatarChange, saving }) {
  ...
  return (
    <div className="bg-white rounded-2xl p-6 shadow-xs border border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-6">
        ...
        <div className="relative group">
          <div className="w-20 h-20 rounded-full border-2 border-slate-200 overflow-hidden flex items-center justify-center bg-gradient-to-br from-violet-500 to-indigo-600 text-white text-2xl font-bold select-none">
            {profile?.avatarUrl ? (
              ...
              getInitials(profile?.fullName || '') //preguntar sobre esto
            )}
          </div>
      ...
      <button
        ...
        className={`w-full sm:w-auto px-5 py-2.5 rounded-xl text-sm font-bold shadow-xs transition-all duration-200 flex items-center justify-center gap-2 border ${
          isEditing
