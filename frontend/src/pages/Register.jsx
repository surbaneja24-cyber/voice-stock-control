  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        // Extracción Quirúrgica del error 422 de Pydantic
        if (Array.isArray(data.detail)) {
          const campoFallido = data.detail[0].loc[1]; // Extrae si fue 'email', 'nombre', etc.
    ...
    <div className={`flex min-h-screen items-center justify-center p-4 py-12 transition-colors duration-300 ${darkMode ? 'bg-slate-950 text-white' : 'bg-slate-50 text-slate-900'}`}>
      ...
      >
            ...
            {formData.password.length > 0 && (
              <div className="mt-2">
                <div className="flex justify-between text-[10px] uppercase font-bold tracking-wider mb-1">
                  ...
                  <span className={`transition-colors ${passStrength.text === t.strength.weak ? "text-red-500" : passStrength.text === t.strength.medium ? "text-yellow-500" : "text-emerald-500"}`}>
                    {passStrength.text}
                  </span>
                </div>
                <div className={`h-1.5 w-full rounded-full overflow-hidden ${darkMode ? 'bg-slate-700' : 'bg-slate-200'}`}>
                  <div className={`h-full rounded-full transition-all duration-300 ease-out ${passStrength.color} ${passStrength.width}`}></div>
                </div>
              </div>
            )}
          </div>
        ...
        <button
          type="submit"
          disabled={loading}
          className={`w-full mt-8 rounded-xl p-3.5 font-bold tracking-wide shadow-lg transition-all text-white flex items-center justify-center ${loading ? 'bg-blue-800/50 cursor-not-allowed opacity-70' : 'bg-blue-600 hover:bg-blue-500 hover:shadow-blue-600/20 active:scale-[0.98]'}`}
        >
          {loading ? (
            <>
