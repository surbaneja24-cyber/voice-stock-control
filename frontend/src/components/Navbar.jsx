  ...
  return (
    ...
    >
      <motion.nav
        ...
        className={`block rounded-2xl shadow-xl border overflow-hidden transition-colors duration-300 ${darkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-200'
          }`}
      >
        <div className="h-[60px] flex items-center justify-between px-4 relative z-10">
          ...
          <button
            ...
            className={`w-10 h-10 flex flex-col items-center justify-center gap-1.5 rounded-full transition-colors ${darkMode ? 'hover:bg-slate-800 text-slate-300' : 'hover:bg-slate-100 text-slate-600'
              }`}
          ...
          <div className="flex items-center gap-2 md:gap-3">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 p-2 md:px-3 md:py-2 rounded-full md:rounded-lg border font-bold text-xs uppercase transition-all duration-300 ${darkMode
            ...
            <button
              ...
              className={`hidden md:flex px-5 items-center justify-center h-10 rounded-lg font-semibold text-sm transition-all duration-300 shadow-md ${darkMode
                ...
                <div key={idx} className={`flex flex-col gap-3 p-5 rounded-xl border transition-colors shadow-sm ${item.colorClass}`}>
                  ...
                  <div className="flex flex-col gap-1 mt-auto">
                    {item.links.map((lnk, i) => (
                      <button
                        key={i}
                        onClick={() => { lnk.action(); setIsOpen(false); }}
                        className={`inline-flex items-center gap-2 text-left py-1 font-medium transition-colors ${darkMode ? 'text-slate-400 hover:text-blue-400' : 'text-slate-600 hover:text-blue-600'
                          }`}
                      >
