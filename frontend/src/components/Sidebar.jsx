...
const Sidebar = ({ isOpen, setIsOpen }) => {
  return (
    <aside 
      className={`fixed left-0 top-0 h-screen bg-white border-r border-slate-200 flex flex-col justify-between transition-all duration-300 z-[100] shadow-[4px_0_24px_rgba(0,0,0,0.02)] ${
    ...
    >
      ...
      <div>
        ...
        <nav className="p-4 space-y-2 mt-2 flex flex-col items-center md:items-stretch">
          <a title="Dashboard" href="#" className={`flex items-center ${isOpen ? 'gap-3 px-4' : 'justify-center px-0 w-12'} py-3 hover:bg-slate-50 text-slate-600 hover:text-slate-900 rounded-lg transition-all duration-300`}>
            <FiGrid size={22} className="shrink-0" />
            {isOpen && <span className="font-medium tracking-wide whitespace-nowrap">Dashboard</span>}
          </a>
          ...
          <a title="Historial" href="#" className={`flex items-center ${isOpen ? 'gap-3 px-4' : 'justify-center px-0 w-12'} py-3 hover:bg-slate-50 text-slate-600 hover:text-slate-900 rounded-lg transition-all duration-300`}>
            <FiClock size={22} className="shrink-0" />
            {isOpen && <span className="font-medium tracking-wide whitespace-nowrap">Historial</span>}
          </a>
